...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def get_action(self, obs: np.ndarray) -> np.ndarray:
        ...
        observation_tensor = torch.FloatTensor(observation).to(ptu.device)
    ...
    def update(self, observations, actions):
        ...
        observations_tensor = torch.FloatTensor(observations).to(ptu.device)
        actions_tensor = torch.FloatTensor(actions).to(ptu.device)
        distribution = self.forward(observations_tensor)

        if self.discrete:
            logits = distribution.logits  
            loss = F.cross_entropy(logits, actions_tensor.long())
        else:
            predicted_actions = distribution.mean 
            loss = -distribution.log_prob(actions_tensor).mean()
